...
# methods/rate_analysis.R
# 染色体数目进化速率分析脚本（修正版，兼容 ggtree 分支着色）
...
# 计算每个分支的速率（单位时间的染色体数目变化量）
compute_branch_rates <- function(tree_file, states_file) {
  ...
  for (i in seq_len(nrow(edge_df))) {
    ...
    # 速率 = 状态差 / 分支长度
    if (!is.na(edge_df$branch_length[i]) && edge_df$branch_length[i] > 0) {
      edge_df$rate[i] <- (edge_df$child_state[i] - edge_df$parent_state[i]) / edge_df$branch_length[i]
    } else {
      edge_df$rate[i] <- NA
    }
  }
  write.csv(edge_df, file = "branch_rates.csv", row.names = FALSE)
  return(edge_df)
}

# 可视化速率分布和树上速率
plot_branch_rates <- function(edge_df, tree_file = "prepared_tree.nwk") {
  # 速率分布直方图
  p1 <- ggplot(edge_df, aes(x = rate)) +
    geom_histogram(bins = 30, fill = "#377eb8", color = "black") +
    theme_minimal() +
    labs(title = "Branch Rate Distribution", x = "Rate (Δchromosome / time)", y = "Count")
  ggsave("branch_rate_histogram.pdf", p1, width = 6, height = 4)
  # 树上速率热图（分支着色）
  if (requireNamespace("ggtree", quietly = TRUE)) {
    library(ggtree)
    ...
    # parent-child 坐标合并
    seg_df <- merge(
      tree_data[, c("parent", "node", "x", "y")],
      tree_data[, c("node", "x", "y")],
      by.x = "node", by.y = "node", suffixes = c("_child", "_parent")
    )
    # seg_df$parent 是 parent, seg_df$node 是 child
    # 为了兼容，重命名
    names(seg_df)[names(seg_df) == "parent"] <- "parent"
    names(seg_df)[names(seg_df) == "node"] <- "child"
    # 合并速率
    seg_df <- merge(seg_df, edge_df[, c("parent", "child", "rate")],
                    by = c("parent", "child"), all.x = TRUE)
    # 绘制分支着色
    p2 <- p2 + geom_segment(
      data = seg_df[!is.na(seg_df$rate), ],
      aes(x = x_parent, y = y_parent, xend = x_child, yend = y_child, color = rate),
      size = 1
    ) +
      scale_color_gradient2(low = "blue", mid = "white", high = "red", midpoint = 0) +
      theme_tree2() +
      labs(title = "Branch Rate Heatmap", color = "Rate")
    ggsave("branch_rate_tree_heatmap.pdf", p2, width = 8, height = 6)
  }
}
...
if (sys.nframe() == 0) {
  ...
  if (length(args) < 2) {
    cat("用法: Rscript methods/rate_analysis.R prepared_tree.nwk bayesian_synteny_states.csv\n")
    quit(status = 1)
  }
  tree_file <- args[1]
  states_file <- args[2]
  edge_df <- compute_branch_rates(tree_file, states_file)
  plot_branch_rates(edge_df, tree_file)
  cat("速率分析与可视化完成，结果已保存为 branch_rates.csv, branch_rate_histogram.pdf, branch_rate_tree_heatmap.pdf\n")
}
